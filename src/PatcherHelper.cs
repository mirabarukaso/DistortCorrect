...
//PostCall系バグ修正＋Lastフックタイプ追加：2018/01/31＠nn
//WrapPostCallRetフックタイプ追加：2018/05/18＠nn フックの確実性は高いですが、他のPatcherとの同じターゲットメソッドでの干渉を考慮しない方法なので注意
...
internal static class PatcherHelper
{
        ...
        AssemblyDefinition calleeAssembly, string calleeTypeName, string calleeMethodName)
    {
        ...
        if (hookType == HookType.WrapPostCallRet)
        {
            WrapPostCallRet(targetAssembly.MainModule, targetTypeDefinition, targetMethod, calleeMethod);
            return;
        }
    ...
    public enum HookType
    {
        ...
        WrapPostCallRet,    // ラッパー関数作成タイプのExPostCallRet。PostCall系が上手く行かない時用
    }
